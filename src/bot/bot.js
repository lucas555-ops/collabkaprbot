...
if (payload?.type === 'bxo') {
  ...
  bot.command('help', async (ctx) => {
    try { await clearExpectText(ctx.from.id); } catch (_) {}

    const text = [
      '❓ <b>Collabka — UGC/Collab CRM в Telegram</b>',
      '',
      '<b>Для Creator’ов</b>',
  ...
  bot.command('paysupport', async (ctx) => {
    ...
    const fallback = [
      '💬 <b>Помощь по оплате / Stars</b>',
      'Если что-то пошло не так — напиши администратору (контакт задаётся в ENV: PAY_SUPPORT_TEXT).',
      '',
      '<b>Что указать в сообщении:</b>',
      '• что покупал (PRO / Brand Pass / Plan / Featured / Matching)',
      '• примерное время оплаты',
      '• скрин чека (если есть)',
      '• свой @username и кратко что случилось'
    ].join('\n');
