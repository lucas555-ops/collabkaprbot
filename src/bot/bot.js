...
async function renderGwEntries(ctx, ownerUserId, gwId, page = 0) {
  ...
  if (typeof db.listGiveawayEntriesPage !== 'function') {
    await ctx.editMessageText('⚠️ Нужен апдейт базы: listGiveawayEntriesPage (см. src/db/queries.js).', {
      parse_mode: 'HTML',
      reply_markup: new InlineKeyboard().text('⬅️ К статистике', `a:gw_stats|i:${gwId}`)
    });
    return;
  }
...
💡 Нажми на участника, чтобы открыть “почему не прошёл”.`;
  ...
  // Keyboard: participant rows + navigation + tools
  const kb = new InlineKeyboard();
...
function iconForCheckStatus(st) {
  if (st === 'ok') return '✅';
  if (st === 'no') return '❌';
  return '❔';
}

function explainCheckStatus(st) {
  if (st === 'ok') return 'подписан(а)';
  if (st === 'no') return 'нет подписки';
  return 'бот не смог проверить (нет доступа/не админ)';
}

async function renderGwEntryDetail(ctx, ownerUserId, gwId, entryUserId, page = 0, justChecked = false) {
  const gOwner = await db.getGiveawayForOwner(gwId, ownerUserId);
  if (!gOwner) return ctx.answerCallbackQuery({ text: 'Нет доступа.' });

  const user = await db.getUserById(Number(entryUserId));
  if (!user?.tg_id) {
    await ctx.editMessageText('⚠️ Не вижу TG ID этого участника.', {
      parse_mode: 'HTML',
      reply_markup: new InlineKeyboard().text('⬅️ К участникам', `a:gw_entries|i:${gwId}|p:${page}`)
    });
    return;
  }

  const tgId = Number(user.tg_id);
  const uname = user.tg_username ? `@${escapeHtml(String(user.tg_username))}` : `<code>${tgId}</code>`;

  const check = await doEligibilityCheck(ctx, gwId, tgId);
  const resByChat = new Map((check.results || []).map(r => [String(r.chat), String(r.status)]));

  // Main + sponsors labels
  const gInfo = await db.getGiveawayInfoForUser(gwId);
  const mainChat = gInfo?.published_chat_id ?? gInfo?.published_chat ?? gInfo?.channel_id ?? null;
  const sponsors = await db.listGiveawaySponsors(gwId);

  const lines = [];

  if (mainChat) {
    const chatKey = String(mainChat);
    const st = resByChat.get(chatKey) || 'unknown';
    lines.push(`${iconForCheckStatus(st)} <b>Основной канал</b>: <code>${escapeHtml(chatKey)}</code> — ${escapeHtml(explainCheckStatus(st))}`);
  } else {
    lines.push(`❔ <b>Основной канал</b>: не задан (конкурс не опубликован?)`);
  }

  for (const s of sponsors) {
    const txt = String(s.sponsor_text || '').trim();
    const chatKey = sponsorToChatId(txt);
    if (!chatKey) {
      lines.push(`❔ <b>Спонсор</b>: ${escapeHtml(txt)} — не удалось распарсить`);
      continue;
    }
    const st = resByChat.get(String(chatKey)) || 'unknown';
    lines.push(`${iconForCheckStatus(st)} <b>Спонсор</b>: ${escapeHtml(txt)} — ${escapeHtml(explainCheckStatus(st))}`);
  }

  const verdict = check.isEligible ? '✅ <b>Подтверждено</b>' : '⚠️ <b>Не подтверждено</b>';
  const note = justChecked ? '

✅ Обновил статус (последняя проверка только что).' : '';
...
💡 Если видишь ❔ — добавь бота админом в канал(ы), иначе Telegram не даст проверить подписку.${note}`;

  const kb = new InlineKeyboard()
    .text('🔄 Проверить сейчас', `a:gec|i:${gwId}|uid:${Number(entryUserId)}|p:${Number(page || 0)}`)
    .row()
    .text('⬅️ К участникам', `a:gw_entries|i:${gwId}|p:${Number(page || 0)}`)
    .text('⬅️ К статистике', `a:gw_stats|i:${gwId}`);
...
// Giveaways: load sponsors from folder
    ...
    if (p.a === 'a:gec') {
      ...
      const usr = await db.getUserById(entryUserId);
      const tgId = Number(usr?.tg_id || 0);
      if (!tgId) return ctx.answerCallbackQuery({ text: 'Не вижу TG ID участника.' });

      await ctx.answerCallbackQuery({ text: 'Проверяю…' });
      const check = await doEligibilityCheck(ctx, gwId, tgId);
      await db.setEntryEligibility(gwId, entryUserId, check.isEligible);
      await db.auditGiveaway(gwId, g.workspace_id, u.id, 'gw.owner_check_user', { entryUserId, tgId, isEligible: check.isEligible, unknown: check.unknown });

      await renderGwEntryDetail(ctx, u.id, gwId, entryUserId, page, true);
      return;
    }
